...
const j=(u,o={})=>fetch(u,o).then(async r=>{let x=await r.json();if(!r.ok){let d=x.detail;if(Array.isArray(d))d=d.map(v=>`${(v.loc||[]).slice(1).join('.')||'field'}: ${v.msg}`).join('\n');throw Error(d||r.statusText)}return x});const n=document.querySelector('#notice');let allEntities=[],points=[],commands=[],sortKey='name',sortDirection=1;const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));function say(x){n.textContent=x;n.style.display='block'}
function memberFor(text){return ('HA_'+text.toUpperCase().replace(/[^A-Z0-9]+/g,'_').replace(/^_+|_+$/g,'')).slice(0,24).replace(/_+$/,'')}
function chooseEntity(){let x=allEntities.find(x=>x.entity_id===entity.value);if(x){if(!name.value)name.value=x.name;if(!member.value)member.value=memberFor(x.name)}}function renderEntities(){let q=entitySearch.value.trim().toLowerCase(),mapped=new Set(points.filter(x=>!x.removed).map(x=>x.entity_id)),shown=allEntities.filter(x=>!mapped.has(x.entity_id)&&(!q||x.entity_id.toLowerCase().includes(q)||x.name.toLowerCase().includes(q)));entity.innerHTML=shown.map(x=>`<option value="${esc(x.entity_id)}">${esc(x.name)} — ${esc(x.entity_id)} (${esc(x.state)})</option>`).join('')||'<option value="">No unmapped entities match</option>';entityCount.textContent=`${shown.length} available · ${mapped.size} already mapped`;chooseEntity()}
